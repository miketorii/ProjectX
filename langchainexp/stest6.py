...
#    action_input_pattern = re.compile(r"(?i)action\s*_*input\s*:\s*([^\n]+)", re.MULTILINE)
    ...
    #action_pattern = re.compile(r"(?i)action\s*:\s*([^\n]+)", re.MULTILINE)
    action_pattern = re.compile(r"(?i)action\s*:\s*(.*(?<!\s))", re.MULTILINE)
...
Do not guess or assume the tool results.
Instead, provide a structured output that includes the action and action_input.
