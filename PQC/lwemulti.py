...
n = 1024
q = 40961
m = 32      # the number of bits for shared key
sigma = 8 / math.sqrt(2*math.pi)

def main():
    print("---start multi bits lwe---")
    
    M = uniform_distribution(q, (n,n))
    print(M)

    # Alice
    sA = np.matrix(normal_distribution((n,m)))
    eA = np.matrix(normal_distribution((n,m)))
    ...
    # Bob
    sB = np.matrix(normal_distribution((n,m)))
    eB = np.matrix(normal_distribution((n,m)))
    ...
    # Bob key
    eB_prime = normal_distribution((m,m))
    ...
    # Alice key
    eA_prime = normal_distribution((m,m))
